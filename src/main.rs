...
use ironclaw::{
    ...
    cli::{
        Cli, Command, run_approvals_command, run_backup_command, run_browser_command, run_cron_command, run_goal_command, run_mcp_command, run_memory_plane_command,
...
async fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Goal(goal_cmd)) => {
            ...
            return run_goal_command(goal_cmd.clone()).await;
        }
        Some(Command::Plan(plan_cmd)) => {
            tracing_subscriber::fmt()
                .with_env_filter(
                    EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("warn")),
                )
        ...
        Some(Command::Browser(browser_cmd)) => {
            ...
            return run_browser_command(browser_cmd.clone()).await;
        }
        ...
        Some(Command::Approvals(approvals_cmd)) => {
            tracing_subscriber::fmt()
                .with_env_filter(
                    EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("warn")),
                )
            ...
            return run_approvals_command(approvals_cmd.clone()).await;
        }
        Some(Command::Doctor) => {
