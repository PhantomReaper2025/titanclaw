...
pub async fn start_server(
    ...
    let protected = Router::new()
        ...
        .route(
            "/api/goals/{id}/policy-decisions",
            get(goal_policy_decisions_list_handler),
        )
...
async fn update_owned_goal_status_and_load(
    state: &GatewayState,
    store: &dyn Database,
    goal_id: Uuid,
    status: GoalStatus,
    ...
    store
        .update_goal_status(goal.id, status)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn goals_complete_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = state.store.as_ref().ok_or((
...
async fn goal_policy_decisions_list_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = state.store.as_ref().ok_or((
    ...
    let goal = load_owned_goal(state.as_ref(), store.as_ref(), goal_id).await?;

    let mut decisions = store
        .list_policy_decisions_for_goal(goal.id)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn update_owned_plan_status_and_load(
    state: &GatewayState,
    store: &dyn Database,
    plan_id: Uuid,
    status: PlanStatus,
    ...
    store
        .update_plan_status(plan.id, status)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn plans_status_update_handler(
    ...
    let plan_id = Uuid::parse_str(&id)
        .map_err(|_| (StatusCode::BAD_REQUEST, "Invalid plan ID".to_string()))?;
    let updated =
        update_owned_plan_status_and_load(state.as_ref(), store.as_ref(), plan_id, body.status)
            .await?;

    Ok(Json(serde_json::json!({ "plan": updated })))
}

async fn plans_complete_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = state.store.as_ref().ok_or((
...
async fn plans_supersede_handler(
    State(state): State<Arc<GatewayState>>,
    Path(id): Path<String>,
) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let store = state.store.as_ref().ok_or((
...
async fn plan_execution_attempts_list_handler(
    ...
    let mut attempts = store
        .list_execution_attempts_for_plan(plan.id)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    attempts.sort_by_key(|a| (a.started_at, a.id));
    attempts.reverse();
