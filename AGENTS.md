...
- Autonomy Control Plane v1 groundwork (internal persistence):
  ...
  - web gateway now exposes user-scoped autonomy goal/plan APIs for create + inspection + status updates (including convenience lifecycle aliases like complete/abandon/supersede), plus plan-step APIs for list/create/detail/status and atomic replace (`/api/plans/{id}/steps`, `/api/plans/{id}/steps/replace`, `/api/plan-steps/{id}`, `/api/plan-steps/{id}/status`)
  ...
  - CLI now exposes `titanclaw goal`, `titanclaw plan`, and `titanclaw plan-step` subcommands for user-scoped create/list/show/set-status workflows plus convenience lifecycle aliases (`goal complete|abandon`, `plan complete|supersede`), `plan replan` (with optional `--copy-steps`), and atomic step replacement (`plan-step replace`)
