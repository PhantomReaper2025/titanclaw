...
| Memory Plane v2 groundwork (types + persistence schema) | 🚧 | Added Phase 2 typed memory-plane domain types (`src/agent/memory_plane.rs`), new database subtrait scaffolding (`AutonomyMemoryStore`) composed into `Database`, dual-backend Postgres/libSQL CRUD foundations for memory records/events/procedural playbooks/consolidation runs, and schema migrations/scaffolding (`V19`-`V23` + libSQL schema mirror updates). Runtime memory writes, consolidation loop, retrieval composer integration, and memory-plane gateway/CLI surfaces remain pending Phase 2 slices. |
| Memory Plane v2 write-policy + rollout flags scaffolding | 🚧 | Added default-off Phase 2 runtime config flags (`AUTONOMY_MEMORY_PLANE_V2`, `AUTONOMY_MEMORY_RETRIEVAL_V2`, `AUTONOMY_MEMORY_CONSOLIDATION_V2` + interval/batch/TTL/playbook knobs) in `settings` + `AgentConfig` env parsing with tests, and added an internal deterministic `MemoryWritePolicyEngine` classifier module with unit tests for target mapping/provenance requirements/high-impact low-confidence rejection. Runtime write call-site integration remains pending. |
| Memory Plane v2 runtime episodic write integration (initial, flag-gated) | 🚧 | Added a shared classifier-backed memory record persistence helper and initial `AUTONOMY_MEMORY_PLANE_V2` runtime write call-sites for worker plan step outcomes, worker verifier outcomes, worker automatic replan events, and worker/chat policy-decision paths (dispatcher + approval resume + worker preflight), all best-effort/fail-open and mirrored into `autonomy_memory_records`. Retrieval composer, consolidation loop, and broader runtime coverage remain pending. |
| Memory Plane v2 scheduler/routine write integration (flag-gated) | 🚧 | Extended `AUTONOMY_MEMORY_PLANE_V2` runtime writes into scheduler tool-subtask execution outcomes (including local-fallback/remote-path hints) and routine engine run summaries, plus procedural playbook-candidate seed records after repeated successful routine runs. Consolidation, retrieval composer, and memory-plane inspection surfaces remain pending. |
| Memory Plane v2 consolidation loop skeleton (flag-gated, supervised) | 🚧 | Added `MemoryConsolidator` with `ConsolidationRun`/`MemoryEvent` processing, batched active episodic-record scans, initial deterministic promotion of `routine_run_summary` records into semantic summaries, and source record demote/archive handling; wired the loop into `Agent::run()` under the existing restartable background supervisor and shutdown signaling via `AUTONOMY_MEMORY_CONSOLIDATION_V2`. Procedural playbook promotion rules and richer semantic/procedural consolidation logic remain pending. |
| Memory Plane v2 procedural playbook promotion/update rules (consolidator) | 🚧 | Extended `MemoryConsolidator` deterministic processing to promote `routine_playbook_candidate` episodic records into `autonomy_procedural_playbooks` (create on first candidate, update confidence/success counters/source IDs on repeated candidates), archive source candidate records, and record consolidation events with `GeneratePlaybook` actions. Retrieval composer integration, richer consolidation heuristics, and memory-plane inspection surfaces remain pending. |
| Memory Plane v2 retrieval composer + planner/replanner integration (flag-gated) | 🚧 | Added `MemoryRetrievalComposer` (task-class inference + task-aware memory/playbook selection + prompt-context rendering) and integrated it into worker initial planning + automatic replanning via transient planner-context injection (`PlannerV1::plan_initial_with_retrieval`) when `AUTONOMY_MEMORY_RETRIEVAL_V2=true`; retrieval failures remain fail-open and existing behavior is preserved when the flag is disabled. Gateway/CLI inspection surfaces and Phase 2 acceptance coverage remain pending. |
| Memory Plane v2 web inspection/ops APIs (user-scoped) | 🚧 | Added gateway memory-plane endpoints for user-scoped records/playbooks inspection and status updates plus consolidation run listing/manual trigger and retrieval preview debugging: `GET /api/memory-plane/records`, `GET /api/memory-plane/records/{id}`, `GET /api/memory-plane/playbooks`, `GET /api/memory-plane/playbooks/{id}`, `POST /api/memory-plane/playbooks/{id}/status`, `GET /api/memory-plane/consolidation/runs`, `POST /api/memory-plane/consolidation/run`, and `POST /api/memory-plane/retrieval/preview`; handlers include ownership checks, filters/sort/pagination validation, and targeted gateway tests. Phase 2 acceptance completion remains pending. |
| Memory Plane v2 CLI inspection/ops parity | 🚧 | Added `titanclaw memory-plane` CLI subcommands for records (`list|show`), playbooks (`list|show|set-status`), consolidation (`runs|run`), and retrieval (`preview`) with user-scoped ownership checks, sort/filter/pagination validation aligned with the gateway, and parser/helper tests in `src/cli/memory_plane.rs`. Phase 2 acceptance completion remains pending. |
