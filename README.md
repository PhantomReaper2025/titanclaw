...
- User-scoped web plan-step APIs for autonomy execution structure management (`GET/POST /api/plans/{id}/steps`, `GET /api/plan-steps/{id}`, `POST /api/plan-steps/{id}/status`)
- CLI autonomy commands: `titanclaw goal {create|list|show|set-status}`, `titanclaw plan {create|list|show|set-status}`, and `titanclaw plan-step {create|list|show|set-status}`
