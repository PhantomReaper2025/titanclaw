...
- CLI autonomy commands: `titanclaw goal {create|list|show|set-status|set-priority|cancel|complete|abandon}` (`goal list --status --limit` supported), `titanclaw plan {create|list|show|set-status|cancel|complete|supersede|replan}` (`plan list --status --limit`; `--copy-steps`, `--steps-file`, and `--steps-json` supported for `replan`), and `titanclaw plan-step {create|replace|list|show|set-status}`
