...
- CLI autonomy commands: `titanclaw goal {create|list|show|set-status|complete|abandon}`, `titanclaw plan {create|list|show|set-status|complete|supersede|replan}` (`--copy-steps`, `--steps-file`, and `--steps-json` supported for `replan`), and `titanclaw plan-step {create|replace|list|show|set-status}`
