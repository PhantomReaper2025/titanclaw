...
| Tooling System v2 foundations (Phase 3) | 🚧 | Phase 3 foundations now include persistence (`V24`-`V27`, libSQL mirror/compat updates, `AutonomyReliabilityStore`, incident/contract-override/reliability-profile CRUD), Tool Contract V2 descriptor inference/overlay helpers and `ToolRegistry` resolver APIs (`user` -> `global` -> inferred), and a conservative internal `ToolReliabilityService` scaffold for manual profile recomputation (score + breaker state) from persisted autonomy execution/policy/incident records plus legacy `tool_failures`; runtime ranking/incident-detector/critic behavior ships in later Phase 3 slices |
