...
- User-scoped web plan revisioning endpoint (`POST /api/plans/{id}/replan`) for next-revision creation with optional superseding of the prior plan
- User-scoped autonomy telemetry inspection APIs (`GET /api/plans/{id}/executions`, `GET /api/goals/{id}/policy-decisions`)
- CLI autonomy commands: `titanclaw goal {create|list|show|set-status}`, `titanclaw plan {create|list|show|set-status|replan}`, and `titanclaw plan-step {create|replace|list|show|set-status}`
