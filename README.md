...
- User-scoped web goal/plan APIs for autonomy inspection + creation + status updates (`GET /api/goals`, `GET /api/goals/{id}`, `GET /api/goals/{id}/plans`, `GET /api/plans?goal_id=...`, `GET /api/plans/{id}`, `POST /api/goals`, `POST /api/plans`, `POST /api/goals/{id}/status`, `POST /api/plans/{id}/status`)
...
- User-scoped web plan revisioning endpoint (`POST /api/plans/{id}/replan`) for next-revision creation with optional superseding of the prior plan and optional step copying into the new revision
- User-scoped autonomy telemetry inspection APIs (`GET /api/plans/{id}/executions`, `GET /api/goals/{id}/policy-decisions`)
- CLI autonomy commands: `titanclaw goal {create|list|show|set-status}`, `titanclaw plan {create|list|show|set-status|replan}` (`--copy-steps` supported), and `titanclaw plan-step {create|replace|list|show|set-status}`
