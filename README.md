...
| Tooling System v2 foundations (Phase 3) | 🚧 | Phase 3 persistence foundations are now landed: schema migrations `V24`-`V27` and libSQL mirror/compatibility updates extend `autonomy_incidents` with dedupe/reliability columns, add `tool_contract_v2_overrides` and `tool_reliability_profiles`, and introduce `AutonomyReliabilityStore` (PostgreSQL + libSQL) plus typed Tool Contract V2 / circuit-breaker reliability domain structs (`src/tools/contract_v2.rs`); resolver/ranking/incident-detector/critic runtime behavior ships in later Phase 3 slices |
