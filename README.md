...
- User-scoped autonomy telemetry inspection APIs (`GET /api/plans/{id}/executions`, `GET /api/plans/{id}/verifications`, `GET /api/goals/{id}/policy-decisions`)
- CLI autonomy commands: `titanclaw goal {create|list|show|set-status|set-priority|cancel|complete|abandon}` (`goal list --status --sort --offset --limit` supported), `titanclaw plan {create|list|show|verifications|set-status|cancel|complete|supersede|replan}` (`plan list --status --sort --offset --limit`; `plan verifications --limit`; `--copy-steps`, `--steps-file`, and `--steps-json` supported for `replan`), and `titanclaw plan-step {create|replace|list|show|set-status}`
