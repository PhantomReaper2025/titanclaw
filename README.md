...
| Tooling System v2 foundations (Phase 3) | 🚧 | Phase 3 foundations now include persistence (`V24`-`V27`, libSQL mirror/compat updates, `AutonomyReliabilityStore`, incident/contract-override/reliability-profile CRUD) plus Tool Contract V2 descriptor inference/overlay helpers and `ToolRegistry` resolver APIs with override precedence (`user` -> `global` -> inferred); runtime ranking/reliability aggregation/incident-detector/critic behavior ships in later Phase 3 slices |
