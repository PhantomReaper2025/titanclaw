...
- CLI exposes `titanclaw goal`, `titanclaw plan`, and `titanclaw plan-step` subcommands (including lifecycle aliases like `goal complete`, `plan supersede`, plus `plan replan --copy-steps` and `plan-step replace`) for direct inspection and manual management of autonomy records in local/single-user workflows.
