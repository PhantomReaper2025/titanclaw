...
- Phase 2 Memory Plane v2 consolidation is now scaffolded and supervised in the agent runtime (`AUTONOMY_MEMORY_CONSOLIDATION_V2`): a `MemoryConsolidator` loop records consolidation runs/events, batches active episodic records, and currently performs deterministic initial promotion of routine summaries into semantic summary records while archiving/demoting processed sources.
